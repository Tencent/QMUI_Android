...
open class EmojiEditText(
    ...
    private val textWatcher = object : TextWatcher {
        ...
        private val pendingRemoveEmojis = mutableListOf<Emoji>()
        private val pendingAddEmojis = mutableListOf<Emoji>()
                ...
                while (current < end) {
                    ...
                    if (emoji != null) {
                        ...
                        if (emojiEnd >= end) {
                            ...
                            pendingRemoveRange.add((end - offset) to (emojiEnd - offset))
                        }
        ...
        override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
            ...
            val model = emojiModel
            if (model != null) {
                var cur: Emoji? = model.end
                val offset = count - before
                val correctStartIndex = start + before
                while (cur != null && cur.start >= correctStartIndex) {
                    cur.start += offset
                    cur = cur.prev
                }
            }
            if (count > 0) {
                ...
                if (insertModel != null) {
                    ...
                    insertModel.map.valueIterator().asSequence().filterTo(pendingAddEmojis) { true }
                }
            }
        ...
        override fun afterTextChanged(s: Editable?) {
            ...
            pendingRemoveRange.forEach {
                s?.removeRange(it.first, it.second)
            }
            pendingRemoveRange.clear()
            pendingRemoveEmojis.forEach {
                s?.removeSpan(it.span)
            }
            pendingRemoveEmojis.clear()
            pendingAddEmojis.forEach {
                s?.setSpan(it.span, it.start, it.start + it.text.length, Spannable.SPAN_INCLUSIVE_EXCLUSIVE)
            }
            pendingAddEmojis.clear()
            fixSelection()
    ...
    fun replaceSelection(toInsert: CharSequence) {
        ...
        if (origin == null) {
            ...
            origin.replace(selectionStart, selectionEnd, toInsert)
        }
    }
    ...
    private fun fixSelection() {
        ...
        if (selectionStart == selectionEnd) {
            ...
            if (start != null && selectionStart > start.start && selectionStart < (start.start + start.text.length)) {
...
class EmojiSpan(drawable: Drawable) : ImageSpan(drawable) {
    override fun getSize(paint: Paint, text: CharSequence?, start: Int, end: Int, fm: Paint.FontMetricsInt?): Int {
        val rect = drawable.bounds
        if (fm != null) {
            val iconHeight = rect.height()
            val fontMetricsHeight = fm.descent - fm.ascent
            if (iconHeight != fontMetricsHeight) {
                val scale = fontMetricsHeight.toFloat() / iconHeight
                rect.left = 0
                rect.top = 0
                rect.right = (rect.width() * scale).toInt()
                rect.bottom = (iconHeight * scale).toInt()
